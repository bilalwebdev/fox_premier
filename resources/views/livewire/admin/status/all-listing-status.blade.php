<div>
    <div class="flex justify-center mb-5">
        <p class="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate">All Status</p>
    </div>
                        ...
                        <div class="">
                            <form wire:submit.prevent="addStatus">
                                <div class="col-md-12">
                                    ...
                                    <input type="text" class="form-control" id="status" wire:model.defer="status" required placeholder="Enter Status">
                                    @error('status')
    ...
    @if ($statuses==NULL)
    <div>
       <span class="alert bg-red-400">No data found!</span>
    </div>
    @else
                ...
                <div class="card-body">
                    @if ($statusID == $list->id)
                        ...
                        <form wire:submit.prevent="update({{ $list->id }})">
                            <input type="text" class="form-control" wire:model.defer="statusUpdate" required>
                            @error('status')
                                <span class="text-red-500">{{ $message }}</span>
                            @enderror
                    ...
                    @else
                        <div class="d-flex justify-content-between">
                            ...
                            <div class="d-flex justify-end">
                                <button wire:click="edit({{ $list->id }})" class="mx-1 text-[#666ee8]"><i
                                        class="ft-edit"></i></button>
                                <button wire:click="$emit('deleteFile',{{$list->id }})" class="text-red-500"><i
...
@push('scripts')
<script>
 window.livewire.on('statusAdded', statusID => {
     $('#model').modal('hide');
 })
</script>
@endpush
